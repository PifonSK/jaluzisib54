...
$('input[name="qw7"]').on('change input', function () {
	...
	$('.gift-img').attr({"src": '/images/qw-8-' + $(this).attr('prize') + '.png',});
});
...
function next() {
    ...
    btnActive(!isValid);
    if (activeSlede[number] === true || number === 3 || number === 4) {
        btnNext.prop('disabled', false);
    } else {
        btnNext.prop('disabled', true);
    }
	...
	if (number === 1) {
        $('.test__block-1').hide();
		$('.test__block-2').show();
		$('.test-slider__wrap').css('width', '791px');
	}
	...
	if (number === 4) {
		for (let i = 1;  i <= $('input[name="Окна"]').val(); i++) {
			$('.qw5-block').append('\
			<div class="qw5">\
				<div class="qw5__number">' + i + ' —</div>\
				<input class="qw5__input" type="text" name="qw5-' + i + '-width" placeholder="Ширина (см)">\
				<input class="qw5__input" type="text" name="qw5-' + i + '-height" placeholder="Высота (см)">\
			</div>\
		')
		}
	}
    ...
    if (number === maxNumber) {
        ...
        $('#r-1').attr({"src": './files/r-1.png.pagespeed.ce.i1o38Z57W0.png',});
		$('.test__block').addClass('finalp');
		$('.test-slider__wrap').css('width', '727px');
    } else {
        setTimeout(function () {
    ...
    $('#test form').on('submit', function (e) {
        ...
        $.ajax({
            ...
            success: function (msg) {
				try {
					yaCounter65984053.reachGoal('QUIZ-SENDFORM');
				} catch(e) {
					console.log(e);
				}
				closeModal();
